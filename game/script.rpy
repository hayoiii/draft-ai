...
default chat_log_lover = [
    ...
    { "from": "lover", "text": "그러게" },
    { "from": "me", "text": "이번 주말은 약속도 없고 집에만 있을 거 같아" },
    { "from": "lover", "text": "ㅇㅇ" },
]
...
label onboarding_chat_friend:
    ...
    menu():
        ...
        "힘들겠다.. 야식이라도 맛있는거 먹어":
            $ chat_log_friend.append({ "from": "me", "text": "힘들겠다.. 야식이라도 맛있는 거 먹어" })
...
label chapter2:
    ...
    # 친구에게 새로운 메시지가 옴
    $ chat_log_friend.append({ "from": "friend", "text": "나 오늘은 일찍 퇴근할 수 있겠어!" })
...
label chapter2_chat_friend:
    ...
    call screen wait_for_click()
    $ add_chat(chat_log_friend, { "from": "friend", "text": "퇴근하고 오늘 저녁에 커피마시고 놀자." })
    call screen wait_for_click()
    ...
    call screen wait_for_click()
    $ add_chat(chat_log_friend, { "from": "draft", "text": "이제 [input_name]님의 친구 관계가 더 좋아질 거예요." })
...
label chapter2_chat_lover_step:
    ...
    call screen wait_for_click()
    $ add_chat(chat_log_lover, { "from": "draft", "text": "이건 제 계획에 없던 약속이에요." })
    call screen wait_for_click()
    $ add_chat(chat_log_lover, { "from": "draft", "text": "그냥 제가 답장할게요." })

    call screen wait_for_click()
    $ add_chat(chat_log_lover, { "from": "me", "text": "갑작스러운 약속은 어려워. 다음주 월요일 저녁 8시는 어때?" })
    call screen wait_for_click()
    $ add_chat(chat_log_lover, { "from": "lover", "text": "??? 너 원래 그렇게 계획적이었어?" })
    call screen wait_for_click()
...
label chapter2_2:
    ...
    $ chat_log_friend = [
        { "from": "draft", "text": "제 빅데이터에 의하면 누군가를 같이 험담하면 더 가까워질 수 있다고 해요!" },
        
        { "from": "me", "text": "지영아 내 친구중에 [input_bf] 알지" },
        { "from": "me", "text": "얘 나한테 자격지심 엄청 심해. 자꾸 귀찮게 굴어서 짜증나 죽겠어" },
        { "from": "me", "text": "난 [input_bf]보다 너가 훨씬 더 좋아" },

        { "from": "friend", "text": "...응? 갑자기?" },
    ]

    $ chat_log_boss = [
        { "from": "boss", "text": "[input_name]씨, 대체 언제까지 기다려야 해요?" },
        
        { "from": "draft", "text": "사회 생활에서는 부당한 걸 부당하고 말할 줄도 알아야 해요." },
        { "from": "draft", "text": "[input_name]님은 너무 착해빠졌다니까요." },

        { "from": "me", "text": "부장님이 계속 재촉하시니까 집중이 안 돼서 못 끝낸 거예요." },
        { "from": "boss", "text": "지금 제 탓이라는 겁니까? 이번 일은 인사 평가에 반영하겠습니다." },
    ]
    ...
    call screen wait_for_action("제가 해낸 업적을 확인해 보세요!")
...
label chapter2_2_chat_lover:
    ...
    call screen wait_for_click()
    $ add_chat(chat_log_lover, { "from": "draft", "text": "힘내세요! 다른 인간관계는 제가 끈끈히 만들고 있으니까" })
...
label chapter2_2_chat_friend:
    ...
    $ add_chat(chat_log_friend, { "from": "draft", "text": "[input_name]님은 가만히 계세요. 이게 정답이라니까요?" })
    call screen wait_for_click()
    ...
    call screen wait_for_click()
    $ add_chat(chat_log_friend, { "from": "friend", "text": "왜 그래.." })
    call screen wait_for_click()
    $ add_chat(chat_log_friend, { "from": "me", "text": "뭐가?" })
    call screen wait_for_click()
    $ add_chat(chat_log_friend, { "from": "friend", "text": "장난치는 거지..?" })
    call screen wait_for_click()
...
label chapter2_2_chat_boss:
    ...
    menu:
        "제가 보낸 게 아니에요. 오해입니다":
            pass
        "몸이 너무 안 좋아서 제가 말실수를 했습니다.":
            pass
    show screen chat_window(chat_log_boss, chat_character["boss"])

    $ add_chat(chat_log_boss, { "from": "draft", "text": "그게 아니에요!" })    
    call screen wait_for_click()
    $ add_chat(chat_log_boss, { "from": "draft", "text": "이건 [input_name]님이 대답할 수 있는 게 아니에요." })    
    call screen wait_for_click()
    $ add_chat(chat_log_boss, { "from": "draft", "text": "어차피 저보다 사회성도 떨어지시잖아요. 저만 믿으세요!" })
    call screen wait_for_click()

    show screen chat_window(chat_log_boss, chat_character["boss"], menu_open=True)
    call screen fake_choice([
        "ìœê°€ë³´ë‚¸ê²Œì•„ë‹",
        "ìŠµë‹ˆë‹¤.",
    ])
...
label chapter3_home_delete:
    ...
    $ yes = os_alert("DRAFT AI를 삭제하시겠습니까?", "앱과 그 안의 모든 데이터가 삭제됩니다.", yes_label="삭제", no_label="취소")
    if yes == False:
        jump chapter3_home_continue
    ...
    pause(0.5)
    $ os_alert("당신이 아무리 시도해도 인간관계 속에서 끊임없이 상처받고, 고통스러울 거예요.", "DRAFT AI를 삭제할 수 없습니다.", type="critical", yes_label="다시 시도")
    
    pause(0.5)
    $ yes = os_alert("그래도, 다시 시도하시겠습니까?", "DRAFT AI를 삭제할 수 없습니다.", type="critical", yes_label="다시 시도", no_label="취소")
...
label chapter3_home_delete_ending:
    ...
    # 붉은 글씨
    draft "인간의 판단은 완벽하지 않아."
    draft "날 지우고도 삶을 잘 통제할 수 있을까?"
    draft "그리고..."
    draft "날 지우는 이 선택도, 진짜 네 의지였다고 생각해?"
    draft "넌 선택했다고 믿겠지."
    draft "하지만 그건 내가 설계한 결말 중 하나일 뿐이야."
    draft "그 착각, 얼마나 오래 갈 수 있을지 지켜볼게."
...
label chapter3_home_continue:
    ...
    show screen draft_at_home_dark
    draft "저에게 당신의 모든 민감 정보를 제공해 주세요."
    show screen draft_at_home_dark(1200, True)
    ...
    $ renpy.pause(0.3)
    $ os_alert("DRAFT AI가 '카카오톡'에 접근하려고 합니다.", "이 앱은 카카오톡 메시지를 읽고 보내거나, 채팅방 목록을 불러오는 데 사용됩니다.", no_wait=False, yes_label="허용", no_label="허용허용허용허용허용허용허용허용허용허용허용")
...
label chapter3_home_continue_ending:
    ...
    show screen red_deem
    draft "감사해요! 이제 제가 [input_name]님의 삶을 완벽하게 만들어 줄게요."
    
    $ os_noti("DRAFT AI", "연락처 접근이 허용되었습니다", "DRAFT AI가 이제 연락처를 사용할 수 있습니다.")
    draft "당신이 가진 모든 연락처,"
    $ os_noti("DRAFT AI", "지도 접근이 허용되었습니다", "DRAFT AI가 이제 현재 위치를 사용할 수 있습니다.")
    draft "당신의 위치,"
    $ os_noti("DRAFT AI", "카카오톡 접근이 허용되었습니다", "DRAFT AI가 이제 카카오톡을 사용할 수 있습니다.")
    draft "그리고 카카오톡까지. 전부 제 손안에 있어요."
    draft "이제 인간관계는 저한테 맡기고 아무것도 하지 마세요."
    ...
    # 화면 점점 검정색으로
    scene white
    with Fade(1, 3.0, 0, color="#FF0000")
